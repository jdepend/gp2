...
public class ObjectTargetExecuteDomainService {
	...
	public void createExecutes(String frequencyType, ObjectTargetVO objectTarget)
			throws TargetException {

		List<ObjectTargetExecute> objectTargetExecutes = new ArrayList<ObjectTargetExecute>();

		TargetFrequencyType targetFrequencyType = TargetFrequencyTypeFactory
				.create(frequencyType);

		List<TargetFrequency> targetFrequencys = targetFrequencyType
				.createTargetFrequencys();
		for (TargetFrequency targetFrequency : targetFrequencys) {
			objectTargetExecutes.add(new ObjectTargetExecute(targetFrequency,
					objectTarget));
		}
	...
	public List<ObjectTargetExecute> getEvaluatingExecutes(String schemeId,
			User user) {
		return loader.getEvaluatingExecutes(schemeId, user);
	}

	public List<ObjectTargetExecute> getOperatedExecutes(String schemeId,
			User user) {
	...
	public void fill(List<FillVO> fills, User user) throws TargetException {
		...
		List<ObjectTargetExecute> executes = executeRepo
				.findByIdIn(new ArrayList<String>(fillMap.keySet()));
		for (ObjectTargetExecute execute : executes) {
			if (!execute.getState().equals(
					ObjectTargetExecuteConstant.State_Filling)) {
				throw new TargetException("执行记录[" + execute.getEvaluateId()
						+ "]的状态为[" + execute.getState() + "]，不能填报");
			}
		...
		applicationContext.publishEvent(new ObjectTargetExecuteFilledEvent(
				executes));
	}
	...
	public void evaluate(List<EvaluateVO> evaluates, User user)
		...
		List<ObjectTargetExecute> executes = executeRepo
				.findByIdIn(new ArrayList<String>(evaluateMap.keySet()));
		for (ObjectTargetExecute execute : executes) {
			if (!execute.getState().equals(
					ObjectTargetExecuteConstant.State_Evaluating)) {
				throw new TargetException("执行记录[" + execute.getEvaluateId()
						+ "]的状态为[" + execute.getState() + "]，不能打分");
			}
		...
		applicationContext.publishEvent(new ObjectTargetExecuteEvalutedEvent(
				executes));
	}
	...
	public List<TargetStatVO> getTargetStats(String schemeId) {
		...
		Map<String, TargetStatVO> targetStatVOMap = this
				.getTargetStatVOs(schemeId);
	...
	private Map<String, TargetStatVO> getTargetStatVOs(String schemeId) {
		...
		for (ObjectTargetExecute execute : executes) {
			...
			if (execute.getState().equals(
					ObjectTargetExecuteConstant.State_Evaluated)) {
				targetStatVO
						.setEvaluatedCount(targetStatVO.getEvaluatedCount() + 1);
			} else if (execute.getState().equals(
					ObjectTargetExecuteConstant.State_Evaluating)) {
				if (execute.getRemindDate().after(sysDate)) {

				} else if (execute.getRemindDate().before(sysDate)
						&& execute.getStartDate().after(sysDate)) {
					targetStatVO.setWaitEvaluateCount(targetStatVO
							.getWaitEvaluateCount() + 1);
				} else if (execute.getStartDate().before(sysDate)
						&& execute.getEndDate().after(sysDate)) {
					targetStatVO.setEvaluatingCount(targetStatVO
							.getEvaluatingCount() + 1);
				} else if (execute.getEndDate().before(sysDate)
						&& execute.getLimitDate().after(sysDate)) {
					targetStatVO.setOverdueEvaluateCount(targetStatVO
							.getOverdueEvaluateCount() + 1);
